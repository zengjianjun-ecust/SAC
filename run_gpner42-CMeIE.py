...
# coding: utf-8
"""
2025年9月15日 22点33分 create run_gpner42-训练.py
此设置从 run_gpner41-训练.py 派生
修改内容 通过 diff run_gpner41-原版.py 和 run_gpner42-原版.py 获得。
修改了： 区分大小写
GPNER41 -> GPNER42
gpner41 -> gpner42
...
from models import GPNER42Model
from trainer import GPNER42Trainer
from data import GPNER42Dataset, GPNER42DataProcessor
...
def get_args():
    ...
    parser.add_argument("--method_name", default='gpner42', type=str,
    ...
    parser.add_argument("--finetuned_model_name", default='gpner42', type=str,
...
#     args.max_grad_norm = 1
    ...
    args.pretrained_model_name = 'RoBERTa_zh_Large_PyTorch'
    args.finetuned_model_name = 'gpner42'
    args.do_train = True
    args.do_predict = False
...
def main(args):
    ...
    if args.do_train:
        ...
        data_processor = GPNER42DataProcessor(args)
        train_samples = data_processor.get_train_sample()
        eval_samples = data_processor.get_dev_sample()
        train_dataset =GPNER42Dataset(train_samples, data_processor, tokenizer, args, mode='train')
        eval_dataset = GPNER42Dataset(eval_samples, data_processor, tokenizer, args, mode='eval')

        model = GPNER42Model(model_class, args)
        trainer = GPNER42Trainer(args=args, model=model, data_processor=data_processor,
